...
#include "compute_body_local_agent.h"
...
ComputeBodyLocalAgent::ComputeBodyLocalAgent(LAMMPS *lmp, int narg, char **arg) :
  Compute(lmp, narg, arg), which(nullptr), index(nullptr), avec(nullptr), bptr(nullptr)
{
  if (narg < 4) error->all(FLERR,"Illegal compute body/local/agent command");
  ...
  if (!avec) error->all(FLERR,"Compute body/local/agent requires atom style body");
  bptr = avec->bptr;

  int indexmax = bptr->noutcol();
  for (int i = 0; i < nvalues; i++) {
    if (which[i] == INDEX && (index[i] < 0 || index[i] >= indexmax))
      error->all(FLERR,"Invalid index in compute body/local/agent command");
  }
...
ComputeBodyLocalAgent::~ComputeBodyLocalAgent()
{
  delete [] which;
  delete [] index;
...
void ComputeBodyLocalAgent::init()
{
    ...
    for (int i = 0; i < nvalues; i++)
      if (which[i] == INDEX && index[i] > 2)
        error->all(FLERR,"Invalid index for non-body particles "
                   "in compute body/local/agent command");
  }
...
void ComputeBodyLocalAgent::compute_local()
{
...
int ComputeBodyLocalAgent::compute_body(int flag)
{
  // perform count
...
void ComputeBodyLocalAgent::reallocate(int n)
{
...
double ComputeBodyLocalAgent::memory_usage()
{
