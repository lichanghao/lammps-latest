...
#include "body_rounded_polyhedron_agent.h"
...
BodyRoundedPolyhedronAgent::BodyRoundedPolyhedronAgent(LAMMPS *lmp, int narg, char **arg) :
  Body(lmp, narg, arg)
{
...
BodyRoundedPolyhedronAgent::~BodyRoundedPolyhedronAgent()
{
  delete icp;
  delete dcp;
...
int BodyRoundedPolyhedronAgent::nsub(AtomVecBody::Bonus *bonus)
{
...
double *BodyRoundedPolyhedronAgent::coords(AtomVecBody::Bonus *bonus)
{
  return bonus->dvalue;
}
...
int BodyRoundedPolyhedronAgent::nedges(AtomVecBody::Bonus *bonus)
{
...
double *BodyRoundedPolyhedronAgent::edges(AtomVecBody::Bonus *bonus)
{
...
int BodyRoundedPolyhedronAgent::nfaces(AtomVecBody::Bonus *bonus)
{
...
double *BodyRoundedPolyhedronAgent::faces(AtomVecBody::Bonus *bonus)
{
  int nvertices = bonus->ivalue[0];
...
double BodyRoundedPolyhedronAgent::enclosing_radius(struct AtomVecBody::Bonus *bonus)
{
  int nvertices = bonus->ivalue[0];
...
double BodyRoundedPolyhedronAgent::rounded_radius(struct AtomVecBody::Bonus *bonus)
{
  int nvertices = bonus->ivalue[0];
  if (nvertices == 1 || nvertices == 2)
...
int BodyRoundedPolyhedronAgent::pack_border_body(AtomVecBody::Bonus *bonus, double *buf)
{
...
int BodyRoundedPolyhedronAgent::unpack_border_body(AtomVecBody::Bonus *bonus, double *buf)
{
...
void BodyRoundedPolyhedronAgent::data_body(int ibonus, int ninteger, int ndouble,
...
int BodyRoundedPolyhedronAgent::pack_data_body(tagint atomID, int ibonus, double *buf)
{
  int i,j,m;
...
int BodyRoundedPolyhedronAgent::write_data_body(FILE *fp, double *buf)
{
  int m = 0;
...
double BodyRoundedPolyhedronAgent::radius_body(int /*ninteger*/, int ndouble,
                                       int *ifile, double *dfile)
{
  int nsub = ifile[0];
  int ned = ifile[1];
...
int BodyRoundedPolyhedronAgent::noutcol()
{
...
int BodyRoundedPolyhedronAgent::noutrow(int ibonus)
{
...
void BodyRoundedPolyhedronAgent::output(int ibonus, int m, double *values)
{
  AtomVecBody::Bonus *bonus = &avec->bonus[ibonus];
...
int BodyRoundedPolyhedronAgent::image(int ibonus, double flag1, double /*flag2*/,
