...
void PairBodyRoundedPolyhedronAgent::init_style()
{
  ...
  if (!avec) error->all(FLERR,"Pair body/rounded/polyhedron/agent requires "
                        "atom style body");
  ...
  if (force->newton_pair == 0)
    error->all(FLERR,"Pair style body/rounded/polyhedron/agent requires "
               "newton pair on");
  ...
  double& energy, double& fpair)
{
  ...
  if (R <= 0) {           // deformation occurs
    fpair = -kn * R * std::sqrt(std::abs(R)) - shift + kna * std::sqrt(std::abs(R));
    if (R <= -hc_threshold) {
      fpair = -kn * (-hc_threshold) * std::sqrt(std::abs(hc_threshold)) - shift + kna * std::sqrt(std::abs(R));
      fpair += -hc_scaling * kn * (R + hc_threshold) * std::sqrt(std::abs(R + hc_threshold));
    }
    e = (0.5 * kn * R + shift) * R * std::sqrt(std::abs(R));
